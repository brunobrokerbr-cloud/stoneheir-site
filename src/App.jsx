...
// Unsplash images — sophisticated, muted tones, architectural elegance
const IMG = {
  heroHome: "https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?w=1920&q=80",  // clean modern building, muted
  heroStone: "https://images.unsplash.com/photo-1486718448742-163732cd1544?w=1920&q=80",  // geometric architecture
  heroAbout: "https://images.unsplash.com/photo-1497366216548-37526070297c?w=1920&q=80",  // clean modern office
  heroContact: "https://images.unsplash.com/photo-1497366811353-6870744d04b2?w=1920&q=80",  // minimalist meeting room
  heroServices: "https://images.unsplash.com/photo-1497215842964-222b430dc094?w=1920&q=80",  // elegant office interior
  heroBlog: "https://images.unsplash.com/photo-1451187580459-43490279c0fa?w=1920&q=80",  // data visualization abstract
  aboutTeam: "https://images.unsplash.com/photo-1600607687939-ce8a6c25118c?w=800&q=80",  // luxury interior neutral tones
  aboutOffice: "https://images.unsplash.com/photo-1600585154340-be6161a56a0c?w=800&q=80",  // elegant architecture
  city1: "https://images.unsplash.com/photo-1512917774080-9991f1c4c750?w=800&q=80",  // modern luxury home exterior
  city2: "https://images.unsplash.com/photo-1613490493576-7fde63acd811?w=800&q=80",  // clean modern villa
};
...
function PublicNav({onLogin}) {
  ...
  const navLink = (to,label) => <Link to={to} style={{fontFamily:serif,fontSize:12,letterSpacing:"0.1em",textTransform:"uppercase",color:page===to?C.g:C.wg,textDecoration:"none",cursor:"pointer",transition:"color 0.3s"}}>{label}</Link>;
  return (
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:100,padding:scrolled?"10px 40px":"16px 40px",display:"flex",justifyContent:"space-between",alignItems:"center",background:scrolled?"rgba(26,31,46,0.95)":"rgba(26,31,46,0.8)",backdropFilter:"blur(20px)",borderBottom:`1px solid rgba(201,168,76,${scrolled?.25:.1})`,transition:"all 0.3s"}}>
      ...
      <div style={{display:"flex",gap:20,alignItems:"center"}}>
        {navLink("/","Início")}{navLink("/servicos","Serviços")}{navLink("/stone","S.T.O.N.E.")}{navLink("/insights","Insights")}{navLink("/sobre","Sobre")}{navLink("/contato","Contato")}
...
function HeroSection() {
  ...
  return (
    <section style={{minHeight:"100vh",display:"flex",flexDirection:"column",justifyContent:"center",alignItems:"center",textAlign:"center",padding:"100px 24px 60px",position:"relative",background:C.ds}}>
      <div style={{position:"absolute",inset:0,backgroundImage:`url(${IMG.heroHome})`,backgroundSize:"cover",backgroundPosition:"center",opacity:0.12}}/>
      ...
      <div style={{position:"relative",zIndex:1,maxWidth:800}}>
        ...
        <div style={{display:"flex",gap:16,justifyContent:"center",opacity:0,animation:"fadeUp 1s 1.4s forwards"}}>
          <Btn gold onClick={()=>navigate("/servicos")}>Nossos Serviços</Btn>
          <Btn outline style={{color:C.g,borderColor:C.g+"66"}} onClick={()=>navigate("/insights")}>Market Insights</Btn>
        </div>
      </div>
      <div style={{position:"absolute",bottom:0,left:0,right:0,display:"grid",gridTemplateColumns:"repeat(4,1fr)",borderTop:`1px solid rgba(201,168,76,0.15)`,background:"rgba(26,31,46,0.85)",backdropFilter:"blur(10px)"}}>
...
function ServicesPage() {
  ...
  return (
    <section style={{padding:"120px 40px 80px",background:C.ds,minHeight:"100vh",position:"relative"}}>
      <div style={{position:"absolute",top:0,left:0,right:0,height:320,backgroundImage:`url(${IMG.heroServices})`,backgroundSize:"cover",backgroundPosition:"center",opacity:0.08,pointerEvents:"none"}}/>
      <div style={{textAlign:"center",maxWidth:600,margin:"0 auto 48px",position:"relative"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:1,maxWidth:1100,margin:"0 auto",background:`rgba(201,168,76,0.1)`}}>
        {services.map((s,i)=>
          <Link key={i} to="/contato" style={{background:C.ds,padding:"32px 28px",transition:"background 0.3s",cursor:"pointer",position:"relative",textDecoration:"none",display:"block"}}>
            ...
            <div style={{display:"inline-block",marginTop:14,fontFamily:mono,fontSize:8,letterSpacing:"0.15em",color:C.g,padding:"4px 10px",border:`1px solid ${C.g}44`}}>SOLICITAR →</div>
          </Link>
        )}
      </div>
    </section>
...
function StonePage() {
  const steps = [
    {l:"S",t:"Source",s:"Coleta de Dados",
      d:"O processo começa com a captura massiva de dados de múltiplas fontes. Nossos algoritmos coletam em tempo real informações de registros públicos de transações, portais imobiliários, dados demográficos do IBGE, índices econômicos do Banco Central, pesquisas de campo proprietárias e dados de geolocalização.",
      details:["Registros públicos de cartórios e prefeituras","Portais imobiliários (ZAP, VivaReal, OLX, Loft)","IBGE — dados demográficos e socioeconômicos","Banco Central — índices e taxas","SECOVI — dados setoriais","Pesquisas proprietárias com 180+ imobiliárias"],
      metric:"2.340+",metricLabel:"FONTES DE DADOS INTEGRADAS"},
    {l:"T",t:"Transform",s:"Processamento por IA",
      d:"Os dados brutos passam por um pipeline de inteligência artificial desenvolvido especificamente para o mercado imobiliário brasileiro. Modelos de machine learning limpam inconsistências, normalizam formatos, identificam outliers e cruzam informações de diferentes fontes para criar um dataset unificado e confiável.",
      details:["Limpeza e normalização de dados brutos","Detecção automática de outliers e anomalias","Cross-referencing entre múltiplas fontes","Modelos preditivos treinados com 500K+ transações","NLP para análise de descrições de imóveis","Geocodificação e enriquecimento geoespacial"],
      metric:"97.3%",metricLabel:"PRECISÃO DOS MODELOS"},
    {l:"O",t:"Outline",s:"Estruturação Analítica",
      d:"Com os dados transformados, aplicamos nosso framework analítico padronizado. Cada análise segue uma estrutura rigorosa que garante consistência entre relatórios e permite comparação direta entre imóveis, bairros e períodos. O framework inclui 6 dimensões de avaliação com pesos calibrados.",
      details:["6 dimensões de avaliação ponderadas","Localização (25%) — tier do bairro, infraestrutura, metrô","Liquidez (20%) — tempo de venda, volume, demanda","Valorização (15%) — histórico, tendência, comparativo","Potencial de Renda (15%) — yield, vacância, demanda","Qualidade (15%) + Risco (10%) — estado, documentação"],
      metric:"6",metricLabel:"DIMENSÕES DE AVALIAÇÃO"},
    {l:"N",t:"Narrate",s:"Tradução em Narrativa",
      d:"Dados e gráficos sem contexto não geram decisões. Na etapa Narrate, nossos analistas — apoiados por IA generativa — transformam os números em narrativas claras, acionáveis e visualmente elegantes. Cada insight é apresentado com o rigor de um relatório de investment bank e a clareza de uma publicação editorial.",
      details:["Narrativa acionável, não apenas descritiva","Visualizações de dados com identidade visual proprietária","Cenários projetados (conservador, base, otimista)","Comparativos com benchmarks do mercado","Linguagem acessível sem perder rigor técnico","Design editorial premium em cada página"],
      metric:"15+",metricLabel:"PÁGINAS POR RELATÓRIO"},
    {l:"E",t:"Evaluate",s:"Controle de Qualidade",
      d:"Nenhum relatório é publicado sem passar pelo nosso checklist de 15 pontos de verificação. Cada dado é conferido, cada projeção é validada, cada visualização é revisada. O relatório recebe um selo de verificação Stoneheir e um número serial único que garante rastreabilidade e autenticidade.",
      details:["Checklist de 15 pontos antes da publicação","Verificação cruzada de todos os dados citados","Validação estatística das projeções","Revisão editorial do texto e visualizações","Selo de verificação Stoneheir","Número serial único: SH-ANO-TIPO-SEQUENCIAL"],
      metric:"15",metricLabel:"PONTOS DE VERIFICAÇÃO"},
  ];
  return (
    <section style={{background:C.ds,minHeight:"100vh"}}>
      {/* Hero */}
      <div style={{padding:"120px 40px 60px",textAlign:"center",position:"relative"}}>
        <div style={{position:"absolute",inset:0,background:"radial-gradient(ellipse 60% 40% at 50% 30%, rgba(201,168,76,0.05) 0%, transparent 70%)",pointerEvents:"none"}}/>
        <div style={{position:"relative",zIndex:1}}>
          <div style={{fontFamily:mono,fontSize:9,letterSpacing:"0.5em",color:C.g,marginBottom:16}}>METODOLOGIA PROPRIETÁRIA</div>
          <h1 style={{fontFamily:display,fontSize:"clamp(40px,6vw,64px)",fontWeight:700,color:C.ow,letterSpacing:"0.15em",marginBottom:8}}>
            <span style={{color:C.g}}>S</span>.
            <span style={{color:C.g}}>T</span>.
            <span style={{color:C.g}}>O</span>.
            <span style={{color:C.g}}>N</span>.
            <span style={{color:C.g}}>E</span>.
          </h1>
          <div style={{fontFamily:display,fontStyle:"italic",fontSize:18,color:C.wg,marginBottom:8}}>Framework</div>
          <div style={{width:60,height:2,background:C.g,margin:"16px auto"}}/>
          <p style={{fontFamily:serif,fontSize:15,color:C.wg,maxWidth:600,margin:"0 auto",lineHeight:1.8}}>
            Cinco etapas que transformam dados brutos em inteligência imobiliária acionável.
            Cada relatório Stoneheir passa por este processo rigoroso que garante precisão, consistência e clareza.
          </p>
        </div>
      </div>
      ...
      <div style={{maxWidth:900,margin:"0 auto",padding:"0 40px 80px"}}>
        {steps.map((s,i)=>
          <div key={i} style={{marginBottom:48,padding:"40px",background:C.ch,borderRadius:8,border:`1px solid ${C.g}15`,position:"relative",overflow:"hidden"}}>
            {/* Background letter */}
            <div style={{position:"absolute",right:-20,top:-30,fontFamily:display,fontSize:200,fontWeight:700,color:`${C.g}06`,lineHeight:1,pointerEvents:"none"}}>{s.l}</div>
            <div style={{position:"relative",zIndex:1}}>
              {/* Header */}
              <div style={{display:"flex",alignItems:"center",gap:20,marginBottom:20}}>
                <div style={{width:64,height:64,border:`2px solid ${C.g}`,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:display,fontSize:28,fontWeight:700,color:C.g,background:C.ds,flexShrink:0}}>{s.l}</div>
                <div>
                  <div style={{fontFamily:mono,fontSize:8,letterSpacing:"0.3em",color:C.gd,marginBottom:4}}>ETAPA {String(i+1).padStart(2,"0")}</div>
                  <h2 style={{fontFamily:display,fontSize:28,fontWeight:700,color:C.ow}}>{s.t}</h2>
                  <div style={{fontFamily:mono,fontSize:10,letterSpacing:"0.15em",color:C.g}}>{s.s.toUpperCase()}</div>
                </div>
              </div>
              {/* Description */}
              <p style={{fontFamily:serif,fontSize:14,color:C.wg,lineHeight:1.8,marginBottom:20}}>{s.d}</p>
              {/* Details grid */}
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px 24px",marginBottom:20}}>
                {s.details.map((d,j)=>
                  <div key={j} style={{display:"flex",alignItems:"baseline",gap:8,padding:"6px 0"}}>
                    <span style={{color:C.g,fontSize:8,flexShrink:0}}>◆</span>
                    <span style={{fontFamily:serif,fontSize:12,color:C.sv,lineHeight:1.5}}>{d}</span>
                  </div>
                )}
              </div>
              {/* Metric */}
              <div style={{display:"inline-flex",alignItems:"center",gap:16,padding:"12px 24px",background:C.ds,borderRadius:4,border:`1px solid ${C.g}33`}}>
                <div style={{fontFamily:mono,fontSize:28,fontWeight:700,color:C.g}}>{s.metric}</div>
                <div style={{fontFamily:mono,fontSize:8,letterSpacing:"0.15em",color:C.st}}>{s.metricLabel}</div>
              </div>
            </div>
          </div>
        )}
        ...
        {/* CTA */}
        <div style={{textAlign:"center",padding:"48px 0",borderTop:`1px solid ${C.g}22`}}>
          <div style={{fontFamily:display,fontSize:24,fontWeight:600,color:C.ow,marginBottom:8}}>Quer ver o S.T.O.N.E. Framework em ação?</div>
          <p style={{fontFamily:serif,fontSize:14,color:C.wg,marginBottom:24}}>Solicite um relatório demonstrativo gratuito e veja a diferença que dados de qualidade fazem na sua decisão.</p>
          <Link to="/contato" style={{display:"inline-block",padding:"12px 32px",background:C.g,fontFamily:serif,fontSize:14,fontWeight:600,color:C.ds,cursor:"pointer",letterSpacing:"0.05em",textDecoration:"none"}}>Solicitar Demonstração</Link>
        </div>
      </div>
    </section>
  );
...
function AboutPage() {
  return (
    <section style={{background:C.ds,minHeight:"100vh"}}>
      {/* Hero */}
      <div style={{padding:"120px 40px 60px",textAlign:"center",position:"relative"}}>
        <div style={{position:"absolute",inset:0,backgroundImage:`url(${IMG.heroAbout})`,backgroundSize:"cover",backgroundPosition:"center",opacity:0.08,pointerEvents:"none"}}/>
        <div style={{position:"relative"}}>
        <div style={{fontFamily:mono,fontSize:9,letterSpacing:"0.5em",color:C.g,marginBottom:16}}>QUEM SOMOS</div>
        <h1 style={{fontFamily:display,fontSize:"clamp(32px,5vw,48px)",fontWeight:700,color:C.ow,marginBottom:8}}>A inteligência por trás<br/>das melhores decisões imobiliárias</h1>
        <div style={{width:60,height:2,background:C.g,margin:"16px auto"}}/>
        <p style={{fontFamily:serif,fontSize:15,color:C.wg,maxWidth:600,margin:"0 auto",lineHeight:1.8}}>
          A Stoneheir nasceu da convicção de que o mercado imobiliário brasileiro merece o mesmo nível de inteligência analítica disponível nos mercados financeiros mais sofisticados do mundo.
        </p>
        </div>
      </div>
      ...
      <div style={{maxWidth:900,margin:"0 auto",padding:"0 40px 80px"}}>
        {/* Team image */}
        <div style={{marginBottom:48,borderRadius:8,overflow:"hidden",height:280,backgroundImage:`url(${IMG.aboutTeam})`,backgroundSize:"cover",backgroundPosition:"center",border:`1px solid ${C.g}22`}}/>
        {/* Origin story */}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:48,marginBottom:60}}>
          <div>
            <div style={{fontFamily:mono,fontSize:9,letterSpacing:"0.3em",color:C.g,marginBottom:12}}>NOSSA HISTÓRIA</div>
            <p style={{fontFamily:serif,fontSize:14,color:C.wg,lineHeight:1.8,marginBottom:16}}>
              Fundada como braço de inteligência do ecossistema Realty Concierge, a Stoneheir foi criada para resolver um problema claro: a falta de dados confiáveis e análises rigorosas no mercado imobiliário brasileiro.
            </p>
            <p style={{fontFamily:serif,fontSize:14,color:C.wg,lineHeight:1.8,marginBottom:16}}>
              Enquanto investidores de ações têm acesso a Bloomberg, Reuters e análises de dezenas de corretoras, quem investe em imóveis historicamente dependeu de "achismo", intuição e dados fragmentados. A Stoneheir existe para mudar isso.
            </p>
            <p style={{fontFamily:serif,fontSize:14,color:C.wg,lineHeight:1.8}}>
              Combinamos inteligência artificial de ponta com curadoria humana especializada para produzir relatórios que têm a profundidade de um research de banco de investimento e a clareza visual de uma publicação editorial premium.
            </p>
          </div>
          <div>
            <div style={{fontFamily:mono,fontSize:9,letterSpacing:"0.3em",color:C.g,marginBottom:12}}>O NOME</div>
            <div style={{padding:32,background:C.ch,borderRadius:8,border:`1px solid ${C.g}15`,marginBottom:20}}>
              <div style={{fontFamily:display,fontSize:32,fontWeight:700,color:C.ow,letterSpacing:"0.15em",marginBottom:8}}>STONEHEIR</div>
              <div style={{width:40,height:2,background:C.g,marginBottom:16}}/>
              <p style={{fontFamily:serif,fontSize:13,color:C.wg,lineHeight:1.7,marginBottom:12}}>
                <span style={{color:C.g,fontWeight:600}}>Stone</span> — Solidez, fundação, a base inabalável sobre a qual se constrói patrimônio.
              </p>
              <p style={{fontFamily:serif,fontSize:13,color:C.wg,lineHeight:1.7}}>
                <span style={{color:C.g,fontWeight:600}}>Heir</span> — Herdeiro, legado, a riqueza que atravessa gerações.
              </p>
            </div>
            <p style={{fontFamily:serif,fontStyle:"italic",fontSize:13,color:C.sv,lineHeight:1.7}}>
              "O nome carrega nossa essência: fornecer a base sólida de dados e inteligência para que nossos clientes construam patrimônio que se perpetue."
            </p>
          </div>
        </div>
        ...
        <div style={{marginBottom:60}}>
          <div style={{textAlign:"center",marginBottom:32}}>
            <div style={{fontFamily:mono,fontSize:9,letterSpacing:"0.5em",color:C.g,marginBottom:12}}>NOSSOS VALORES</div>
            <h2 style={{fontFamily:display,fontSize:28,fontWeight:600,color:C.ow}}>O que nos guia</h2>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:1,background:`rgba(201,168,76,0.1)`,borderRadius:8,overflow:"hidden"}}>
            {[
              {t:"Rigor Analítico",d:"Cada dado é verificado, cada projeção é validada. Não publicamos sem certeza.",i:"◉"},
              {t:"Clareza",d:"Complexidade nos bastidores, simplicidade na entrega. Dados compreensíveis por todos.",i:"◈"},
              {t:"Independência",d:"Análises imparciais. Nunca vendemos imóveis — vendemos inteligência sobre eles.",i:"△"},
              {t:"Legado",d:"Pensamos em gerações, não em trimestres. Cada decisão impacta patrimônio familiar.",i:"◇"},
              {t:"Inovação Silenciosa",d:"Tecnologia de ponta sem alarde. A IA trabalha nos bastidores, o resultado fala.",i:"□"},
            ].map((v,i)=>
              <div key={i} style={{background:C.ds,padding:"28px 20px",textAlign:"center"}}>
                <div style={{fontFamily:mono,fontSize:22,color:C.g,marginBottom:12}}>{v.i}</div>
                <h3 style={{fontFamily:display,fontSize:15,fontWeight:600,color:C.ow,marginBottom:8}}>{v.t}</h3>
                <p style={{fontFamily:serif,fontSize:11,color:C.wg,lineHeight:1.6}}>{v.d}</p>
              </div>
            )}
          </div>
        </div>
        ...
        {/* Ecosystem */}
        <div style={{padding:40,background:C.ch,borderRadius:8,border:`1px solid ${C.g}15`,textAlign:"center"}}>
          <div style={{fontFamily:mono,fontSize:9,letterSpacing:"0.3em",color:C.g,marginBottom:12}}>ECOSSISTEMA</div>
          <h2 style={{fontFamily:display,fontSize:24,fontWeight:600,color:C.ow,marginBottom:8}}>Parte do ecossistema Realty Concierge</h2>
          <p style={{fontFamily:serif,fontSize:14,color:C.wg,lineHeight:1.7,maxWidth:650,margin:"0 auto 32px"}}>
            A Stoneheir é o braço de inteligência de um dos ecossistemas imobiliários mais sofisticados do Brasil. A Realty Concierge é uma rede de licenciamento premium que reúne as melhores práticas do mercado internacional de real estate, adaptadas para o contexto brasileiro com padrão de excelência inegociável.
          </p>

          {/* RC Logo area */}
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:48,margin:"32px 0",padding:"40px",background:C.ds,borderRadius:8,border:`1px solid ${C.st}22`}}>
            <div style={{textAlign:"center"}}>
              <div style={{width:180,height:180,border:`2px solid ${C.ow}`,display:"flex",flexDirection:"column",justifyContent:"center",padding:"16px 20px",borderTopRightRadius:0,borderBottomLeftRadius:0}}>
                <div style={{fontFamily:"'Inter','Helvetica Neue',sans-serif",fontSize:26,fontWeight:300,letterSpacing:"0.12em",color:C.ow,lineHeight:1.15}}>REALTY</div>
                <div style={{fontFamily:"'Inter','Helvetica Neue',sans-serif",fontSize:26,fontWeight:300,letterSpacing:"0.12em",color:C.ow,lineHeight:1.15}}>CONCIERGE</div>
                <div style={{borderLeft:`2px solid ${C.ow}`,paddingLeft:12,marginTop:16,marginLeft:2}}>
                  <div style={{fontFamily:"'Inter','Helvetica Neue',sans-serif",fontSize:10,letterSpacing:"0.4em",color:C.ow,fontWeight:300}}>BRASIL</div>
                </div>
              </div>
            </div>
            <div style={{width:1,height:120,background:`${C.st}33`}}/>
            <div style={{textAlign:"center"}}>
              <div style={{width:180,height:180,border:`2px solid ${C.ow}`,display:"flex",flexDirection:"column",justifyContent:"center",padding:"16px 20px",borderTopRightRadius:0,borderBottomLeftRadius:0}}>
                <div style={{fontFamily:"'Inter','Helvetica Neue',sans-serif",fontSize:26,fontWeight:300,letterSpacing:"0.12em",color:C.ow,lineHeight:1.15}}>REALTY</div>
                <div style={{fontFamily:"'Inter','Helvetica Neue',sans-serif",fontSize:26,fontWeight:300,letterSpacing:"0.12em",color:C.ow,lineHeight:1.15}}>CONCIERGE</div>
                <div style={{borderLeft:`2px solid ${C.ow}`,paddingLeft:12,marginTop:16,marginLeft:2}}>
                  <div style={{fontFamily:"'Inter','Helvetica Neue',sans-serif",fontSize:10,letterSpacing:"0.4em",color:C.ow,fontWeight:300}}>INTERNATIONAL</div>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Ecosystem companies */}
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:1,background:`${C.st}22`,borderRadius:8,overflow:"hidden",marginTop:32}}>
            {[
              {name:"REALTY CONCIERGE",role:"Licenciamento Premium",desc:"Rede de licenciamento que conecta corretores de alto padrão a uma marca internacional com metodologia, treinamento e ferramentas proprietárias."},
              {name:"STONEHEIR",role:"Real Estate Intelligence",desc:"Braço de inteligência. Análises, relatórios de avaliação, estudos de mercado e scoring de investimento com IA e dados proprietários."},
              {name:"POPLAR",role:"Mercado Acessível",desc:"Marca voltada para imóveis até R$ 1.5M com identidade pop art e abordagem jovem, sem abrir mão de qualidade analítica."},
              {name:"VANTAUR",role:"Investimento Fracionado",desc:"Plataforma de fracionamento imobiliário que democratiza o acesso a investimentos em imóveis premium."},
            ].map((co,i)=>
              <div key={i} style={{background:C.ds,padding:"28px 20px"}}>
                <div style={{fontFamily:display,fontSize:13,fontWeight:700,letterSpacing:"0.12em",color:C.ow,marginBottom:4}}>{co.name}</div>
                <div style={{fontFamily:mono,fontSize:7,letterSpacing:"0.2em",color:C.g,marginBottom:12}}>{co.role.toUpperCase()}</div>
                <p style={{fontFamily:serif,fontSize:11,color:C.wg,lineHeight:1.6}}>{co.desc}</p>
              </div>
            )}
          </div>

          <p style={{fontFamily:serif,fontStyle:"italic",fontSize:12,color:C.sv,textAlign:"center",marginTop:24,lineHeight:1.7}}>
            "Cada empresa do ecossistema opera de forma independente, mas compartilha a mesma obsessão por dados, design e excelência. A Realty Concierge é o coração operacional; a Stoneheir é o cérebro analítico."
          </p>
        </div>
      </div>
    </section>
...
function ContactPage() {
  const [form,setForm]=useState({nome:"",email:"",tel:"",assunto:"",mensagem:""});
  const [sent,setSent]=useState(false);
  const up=(f,v)=>setForm(p=>({...p,[f]:v}));
  const send=()=>{if(form.nome&&form.email&&form.mensagem)setSent(true);};
  const iStyle={width:"100%",padding:"12px 16px",border:`1px solid ${C.st}44`,borderRadius:4,fontFamily:mono,fontSize:12,color:C.ow,background:"transparent",outline:"none"};

  return (
    <section style={{background:C.ds,minHeight:"100vh"}}>
      <div style={{padding:"120px 40px 60px",textAlign:"center"}}>
        <div style={{fontFamily:mono,fontSize:9,letterSpacing:"0.5em",color:C.g,marginBottom:16}}>FALE CONOSCO</div>
        <h1 style={{fontFamily:display,fontSize:"clamp(32px,5vw,48px)",fontWeight:700,color:C.ow,marginBottom:8}}>Entre em contato</h1>
        <div style={{width:60,height:2,background:C.g,margin:"16px auto"}}/>
        <p style={{fontFamily:serif,fontSize:15,color:C.wg,maxWidth:500,margin:"0 auto",lineHeight:1.8}}>
          Quer saber mais sobre nossos relatórios ou solicitar uma análise personalizada? Estamos à disposição.
        </p>
      </div>

      <div style={{maxWidth:900,margin:"0 auto",padding:"0 40px 80px",display:"grid",gridTemplateColumns:"1fr 1fr",gap:48}}>
        {/* Form */}
        <div>
          {sent ? (
            <div style={{padding:48,background:C.ch,borderRadius:8,textAlign:"center"}}>
              <div style={{fontFamily:mono,fontSize:40,color:C.g,marginBottom:16}}>✓</div>
              <h3 style={{fontFamily:display,fontSize:22,fontWeight:600,color:C.ow,marginBottom:8}}>Mensagem enviada</h3>
              <p style={{fontFamily:serif,fontSize:14,color:C.wg}}>Retornaremos em até 24 horas úteis.</p>
            </div>
          ) : (
            <div style={{display:"flex",flexDirection:"column",gap:16}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
                <div>
                  <label style={{display:"block",fontFamily:serif,fontSize:11,fontWeight:600,color:C.wg,marginBottom:4}}>Nome</label>
                  <input style={iStyle} value={form.nome} onChange={e=>up("nome",e.target.value)} placeholder="Seu nome"/>
                </div>
                <div>
                  <label style={{display:"block",fontFamily:serif,fontSize:11,fontWeight:600,color:C.wg,marginBottom:4}}>Telefone</label>
                  <input style={iStyle} value={form.tel} onChange={e=>up("tel",e.target.value)} placeholder="(11) 99999-0000"/>
                </div>
              </div>
              <div>
                <label style={{display:"block",fontFamily:serif,fontSize:11,fontWeight:600,color:C.wg,marginBottom:4}}>Email</label>
                <input style={iStyle} type="email" value={form.email} onChange={e=>up("email",e.target.value)} placeholder="seu@email.com"/>
              </div>
              <div>
                <label style={{display:"block",fontFamily:serif,fontSize:11,fontWeight:600,color:C.wg,marginBottom:4}}>Assunto</label>
                <select style={{...iStyle,cursor:"pointer"}} value={form.assunto} onChange={e=>up("assunto",e.target.value)}>
                  <option value="" style={{background:C.ds}}>Selecione...</option>
                  <option value="relatorio" style={{background:C.ds}}>Solicitar Relatório</option>
                  <option value="demo" style={{background:C.ds}}>Demonstração Gratuita</option>
                  <option value="licenciamento" style={{background:C.ds}}>Licenciamento Realty Concierge</option>
                  <option value="parceria" style={{background:C.ds}}>Parceria Comercial</option>
                  <option value="imprensa" style={{background:C.ds}}>Imprensa</option>
                  <option value="outro" style={{background:C.ds}}>Outro Assunto</option>
                </select>
              </div>
              <div>
                <label style={{display:"block",fontFamily:serif,fontSize:11,fontWeight:600,color:C.wg,marginBottom:4}}>Mensagem</label>
                <textarea style={{...iStyle,minHeight:120,resize:"vertical"}} value={form.mensagem} onChange={e=>up("mensagem",e.target.value)} placeholder="Como podemos ajudar?"/>
              </div>
              <div onClick={send} style={{padding:"14px 32px",background:C.g,textAlign:"center",fontFamily:serif,fontSize:14,fontWeight:600,color:C.ds,cursor:"pointer",borderRadius:4,letterSpacing:"0.05em",marginTop:4}}>
                Enviar Mensagem
              </div>
            </div>
          )}
        ...
        {/* Info */}
        <div>
          <div style={{padding:32,background:C.ch,borderRadius:8,border:`1px solid ${C.g}15`,marginBottom:24}}>
            <div style={{fontFamily:mono,fontSize:9,letterSpacing:"0.3em",color:C.g,marginBottom:16}}>INFORMAÇÕES DE CONTATO</div>
            {[
              {l:"Email",v:"contato@stoneheir.com.br",i:"✉"},
              {l:"Telefone",v:"+55 (11) 9999-0000",i:"☎"},
              {l:"WhatsApp",v:"+55 (11) 9999-0000",i:"◆"},
              {l:"LinkedIn",v:"linkedin.com/company/stoneheir",i:"◈"},
            ].map((item,i)=>
              <div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 0",borderBottom:i<3?`1px solid ${C.st}22`:"none"}}>
                <span style={{fontFamily:mono,fontSize:14,color:C.g,width:24,textAlign:"center"}}>{item.i}</span>
                <div>
                  <div style={{fontFamily:mono,fontSize:8,letterSpacing:"0.15em",color:C.st}}>{item.l.toUpperCase()}</div>
                  <div style={{fontFamily:serif,fontSize:13,color:C.ow}}>{item.v}</div>
                </div>
              </div>
            )}
          </div>

          <div style={{padding:32,background:C.ch,borderRadius:8,border:`1px solid ${C.g}15`}}>
            <div style={{fontFamily:mono,fontSize:9,letterSpacing:"0.3em",color:C.g,marginBottom:16}}>HORÁRIO DE ATENDIMENTO</div>
            <div style={{fontFamily:serif,fontSize:13,color:C.wg,lineHeight:2}}>
              Segunda a Sexta: 9h — 18h<br/>
              Sábado: 9h — 13h<br/>
              Domingo e Feriados: Fechado
            </div>
            <div style={{marginTop:16,padding:"12px 16px",background:C.ds,borderRadius:4}}>
              <div style={{fontFamily:mono,fontSize:8,letterSpacing:"0.12em",color:C.g,marginBottom:4}}>RESPOSTA MÉDIA</div>
              <div style={{fontFamily:mono,fontSize:18,fontWeight:700,color:C.ow}}>{"<"} 4 horas</div>
              <div style={{fontFamily:serif,fontSize:10,color:C.st}}>em horário comercial</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <footer style={{padding:"48px 40px 24px",borderTop:`1px solid rgba(201,168,76,0.1)`,background:C.ds}}>
      <div style={{maxWidth:1000,margin:"0 auto",display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:32}}>
        ...
        <div>
          ...
          {[{l:"Sobre a Stoneheir",p:"/sobre"},{l:"Metodologia S.T.O.N.E.",p:"/stone"},{l:"Contato",p:"/contato"},{l:"LinkedIn",p:null}].map(t=>t.p?<Link key={t.l} to={t.p} style={{display:"block",fontFamily:serif,fontSize:12,color:C.wg,marginBottom:6,cursor:"pointer",textDecoration:"none"}}>{t.l}</Link>:<div key={t.l} style={{fontFamily:serif,fontSize:12,color:C.wg,marginBottom:6,cursor:"pointer"}}>{t.l}</div>)}
        </div>
      </div>
...
export default function StoneHeirApp() {
  ...
  const handleLogin=(u)=>{setUser(u);navigate("/painel");};
  ...
  return (
    <div style={{background:C.ds}}>
      <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500&family=Playfair+Display:ital,wght@0,400;0,600;0,700;1,400&family=Source+Serif+4:ital,wght@0,300;0,400;0,600;1,300;1,400&family=JetBrains+Mono:wght@400;500;700&display=swap" rel="stylesheet"/>
      <ScrollToTop/>
      <PublicNav onLogin={()=>user?navigate("/painel"):navigate("/login")}/>
      <Routes>
        <Route path="/" element={<><HeroSection/><ServicesPage/><StonePage/><PublicFooter/></>}/>
        <Route path="/servicos" element={<><ServicesPage/><PublicFooter/></>}/>
        <Route path="/stone" element={<><StonePage/><PublicFooter/></>}/>
        <Route path="/insights" element={<><BlogPage/><PublicFooter/></>}/>
        <Route path="/sobre" element={<><AboutPage/><PublicFooter/></>}/>
        <Route path="/contato" element={<><ContactPage/><PublicFooter/></>}/>
